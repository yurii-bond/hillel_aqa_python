import requests  # Імпорт бібліотеки requests для взаємодії з API через HTTP запити

base_api_url = "https://qauto.forstudy.space/api"  # Базова URL для всіх запитів до API
# s = requests.session()  # Створення сесії для зберігання даних між HTTP запитами


class Auth():  # Клас для взаємодії з API автентифікації
    ...
    def logout(s: requests.session, request_body: dict = None):
        ...
        # Виконання GET-запиту за вказаним URL і овернення результату:
